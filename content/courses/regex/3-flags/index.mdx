...
In the previous lesson, I used an example string of:

```txt
my channel is deeecode, and I make videos on code
```

and the pattern was `/code/`. This pattern matched only the first occurence of code:

```txt
my channel is deee<hi>code</hi>, and I make videos on code
```
...
```txt
my channel is deee<hi>code</hi>, and I make videos on <hi>code</hi>
```

## The Case Insensitive Flag `i`

By default, your regex patterns are case sensitive. This means: `/hello/` will not match the "Hello" in "Hello, how are you" because the cases are not the same (lower "h" and capital "H" not the same).
...
For example, this pattern `/hello.hi/` will match **"hello hi"**, **"hello_hi"**, **"hello8hi"**, but will not match:
...
```txt
<hi>Aisha</hi> likes to code.
<hi>Aisha</hi> also likes to play basketball
```

Using the caret symbol, called the beginning character, we can choose to match "Aisha" only if it begins the string. We do that by adding the caret symbol at the beginning: `/^Aisha/g`:
...
By applying the `m` flag, `/^Aisha/gm`:
